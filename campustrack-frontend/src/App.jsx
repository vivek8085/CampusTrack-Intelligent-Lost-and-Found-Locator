...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      const initial = Number(
        localStorage.getItem("campustrack_notif_count") || 0
      );
      ...
      ws.onmessage = (ev) => {
        try {
          ...
          window.dispatchEvent(
            new CustomEvent("campustrack:incomingChat", { detail: parsed })
          );
          ...
          if (
            ...
            parsed.from &&
            myEmail &&
            parsed.from.toLowerCase() !== myEmail
          ) {
  ...
  return (
    <div className="app-root relative min-h-screen overflow-hidden">
      ...
      <div
        className="absolute inset-0 animated-gradient -z-20"
        aria-hidden="true"
      />
      <div
        className="absolute -left-28 -top-24 blob bg-purple-400 opacity-40 -z-10"
        aria-hidden="true"
      />
      <div
        className="absolute right-12 top-1/4 blob bg-yellow-300 opacity-30 -z-10"
        aria-hidden="true"
      />
      ...
      <nav className="glass-nav nav-dark text-white px-6 py-3 flex justify-between items-center">
        <div className="flex items-center">
          <h1 className="text-2xl font-semibold tracking-tight brand-animated">
            CampusTrack
          </h1>
        ...
        <div className="flex gap-3 items-center flex-wrap">
          <button
            ...
            className={`nav-btn transition transform duration-200 px-3 py-1 rounded-md text-sm shadow-sm ${
              page === "dashboard" ? "nav-active" : "nav-inactive"
            }`}
          >
          ...
          >
            ...
            {chatUnreadCount > 0 && (
              <span
                className="ml-2 inline-flex items-center justify-center bg-red-600 text-white text-xs font-semibold px-2 py-0.5 rounded-full"
                aria-hidden="true"
              >
                {chatUnreadCount}
              </span>
      ...
      <div className="p-6 max-w-6xl mx-auto mt-8 glass-card">
        ...
        {(page === "report-lost" || page === "report-found") && (
          <div>
            {page === "report-lost" && <LostItemForm />}

            {page === "report-found" && <FoundItemForm />}
          </div>
        )}
      </div>
                ...
                onSelect={async (conv) => {
                  ...
                  try {
                    ...
                    if (partner) {
                      ...
                      window.dispatchEvent(
                        new CustomEvent("campustrack:conversationRead", {
                          detail: { partner },
                        })
                      );
                    }
